...
const updateLoyaltyRuleSchema = createLoyaltyRuleSchema.partial();
...
function isWithinWorkingHours(date = new Date()): boolean {
  const minutes = getMoscowMinutes(date);
...
export async function registerRoutes(
  ...
  router.post("/users/register", async (req, res) => {
    try {
      ...
      const existingByPhone = await storage.getUserByPhone(payload.phone);
      if (existingByPhone) {
  ...
  router.post("/users/login", async (req, res) => {
    try {
      ...
      const user = await storage.validateUserPassword(identifier.trim(), password);
      if (!user) {
    ...
    try {
      ...
      if (!isWithinWorkingHours(now)) {
        return res.status(400).json({
          success: false,
          message: `Заказы принимаются с 10:00 до 22:45 (МСК). Сейчас ${formatMoscowTime(now)}.`,
        });
      }
      ...
      res
        .status(201)
        .json({ success: true, order: newOrder, loyaltyReward });
    } catch (err) {
      handleError(res, err);
    }
  });
  ...
  // ---------- LOYALTY RULES ----------
  router.get("/loyalty-rules", requireAdmin, async (_req, res) => {
    try {
      const rules = await storage.getLoyaltyRules();
      res.json(rules);
    } catch (err) {
      handleError(res, err);
    }
  ...
  router.get("/loyalty-rules/active", async (_req, res) => {
    try {
      const rules = await storage.getLoyaltyRules();
      res.json(rules.filter((r) => r.isActive !== false));
    } catch (err) {
      handleError(res, err);
    }
  });

  router.post("/loyalty-rules", requireAdmin, async (req, res) => {
    try {
      const parsed = createLoyaltyRuleSchema.safeParse(req.body);
      if (!parsed.success) {
        return res.status(400).json({ message: "Некорректные данные правила" });
      }

      const rule = await storage.createLoyaltyRule(parsed.data as any);
  ...
  router.patch("/loyalty-rules/:id", requireAdmin, async (req, res) => {
    try {
      const id = Number(req.params.id);
      if (!Number.isInteger(id) || id <= 0) {
      ...
      const parsed = updateLoyaltyRuleSchema.safeParse(req.body);
      if (!parsed.success) {
        return res.status(400).json({ message: "Некорректные данные правила" });
      }

      const updated = await storage.updateLoyaltyRule(id, parsed.data as any);
      if (!updated) {
  ...
  router.delete("/loyalty-rules/:id", requireAdmin, async (req, res) => {
    try {
      const id = Number(req.params.id);
      if (!Number.isInteger(id) || id <= 0) {
        return res.status(400).json({ message: "Некорректный ID правила" });
      ...
      await storage.deleteLoyaltyRule(id);
      res.json({ success: true });
    } catch (err) {
      handleError(res, err);
    }
