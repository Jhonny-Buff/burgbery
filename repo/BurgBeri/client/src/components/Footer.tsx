...
export function Footer({ logoUrl }: FooterProps) {
  ...
  const { data: vacancies = [] } = useQuery<Vacancy[]>({
    ...
    queryFn: async () => {
      const res = await apiRequest('GET', '/api/vacancies');
      return res.json();
    },
  });

  const fallbackVacancies: Vacancy[] = [
    { id: 1, title: 'Повар', salary: 'от 45 000 ₽' },
    { id: 2, title: 'Курьер', salary: 'от 35 000 ₽' },
  ];

  const vacanciesToShow = vacancies.length > 0 ? vacancies : fallbackVacancies;
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 md:py-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-8">
          <div>
            ...
            <h3 className="text-white mb-2 text-lg" data-testid="text-footer-brand">Бургбери</h3>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2 text-zinc-400 text-sm">
                ...
                <div className="flex flex-col">
                  <a href="tel:+79002877740" className="hover:text-orange-500 transition-colors">+7 (900) 28-777-40</a>
                  <a href="tel:+79181772033" className="hover:text-orange-500 transition-colors">+7 (918) 17-720-33</a>
                </div>
              </div>
              <div className="flex items-center gap-2 text-zinc-400 text-sm">
          ...
          <div>
            ...
            <div className="space-y-3 mb-6">
              {vacanciesToShow.map((vacancy) => (
                ...
                >
                  <p className="text-white text-sm mb-1">{vacancy.title}</p>
                  {vacancy.salary ? (
                    <p className="text-zinc-500 text-xs">{vacancy.salary}</p>
                  ) : null}
                  {vacancy.description ? (
                    <p className="text-zinc-500 text-xs mt-1">{vacancy.description}</p>
                  ) : null}
                </div>
              ))}
            </div>
        ...
        <div className="border-t border-zinc-800 pt-8">
          <p className="text-zinc-500 text-sm text-center">
            © {new Date().getFullYear()} Бургбери. Все права защищены.
          </p>
        </div>
      </div>
    </footer>
